...
static inline ureg hms_hash_pos(hms* h, const char* key){
    ...
    //xor folding like described here: www.isthe.com/chongo/tech/comp/fnv/index.html
...
void hms_grow(hms* h){
    ...
    h->grow_on_elem_count = size_old + size_old / 2; //keep wasted space below 25%
    hms_node* z = h->map;
    while(z!=h->map_end){
        z->key = NULL;
        z++;
